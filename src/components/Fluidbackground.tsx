...
float area(vec2 a, vec2 b, vec2 c){
    float A = length(b-c), B = length(a-c), C = length(a-b), s = 0.5*(A+B+C);
...
void main() {
    ...
    float mixer = cos(uv.x * d + uv.y * a) * 0.5 + 0.5;

    float smoothAmount = clamp(uSoftness * 0.1, 0.0, 0.9);
    mixer = mix(mixer, 0.5, smoothAmount);
  ...
  useEffect(() => {
    ...
    const displayMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uColor1: {
          value: new THREE.Vector3(...Object.values(hextoRGB(config.color1))),
        },
        uColor2: {
          value: new THREE.Vector3(...Object.values(hextoRGB(config.color2))),
        },
